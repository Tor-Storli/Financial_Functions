...
use std::error::Error;
use crate::helpers::{read_varchar, parse_csv_f64, parse_csv_dates, write_f64};
...
/// NPV: sum of discounted cash flows starting at period 1.
...
/// NPV at rate r starting from period 0 — used internally by IRR/XIRR solvers.
...
/// IRR via bisection. Requires at least one positive and one negative cash flow.
...
/// MIRR: closed-form formula matching numpy-financial and Excel exactly.
...
/// XNPV: NPV with actual day-count year fractions from the first date.
...
/// XIRR via bisection using actual day-count year fractions.
...
/// Generic bisection root finder. Assumes f(lo)*f(hi) <= 0.
    ...
    unsafe fn invoke(_: &(), input: &mut DataChunkHandle, output: &mut dyn WritableVector) -> Result<(), Box<dyn Error>> {
        ...
        for i in 0..len {
            ...
            match parse_csv_f64(&raw) {
                Ok(values) if !values.is_empty() => write_f64(&mut out, i, npv_calc(rate, &values)),
                _ => out.set_null(i),
            }
        }
        Ok(())
...
impl VScalar for IrrFunction {
    ...
    unsafe fn invoke(_: &(), input: &mut DataChunkHandle, output: &mut dyn WritableVector) -> Result<(), Box<dyn Error>> {
        ...
        for i in 0..len {
            ...
            match parse_csv_f64(&raw) {
                Ok(values) => write_f64(&mut out, i, calc_irr(&values)),
...
impl VScalar for MirrFunction {
    ...
    unsafe fn invoke(_: &(), input: &mut DataChunkHandle, output: &mut dyn WritableVector) -> Result<(), Box<dyn Error>> {
        ...
        for i in 0..len {
            ...
            match parse_csv_f64(&raw) {
                Ok(values) => write_f64(&mut out, i, calc_mirr(&values, finance_rate, reinvest_rate)),
                Err(_)     => out.set_null(i),
            }
        }
        Ok(())
...
impl VScalar for XnpvFunction {
    ...
    unsafe fn invoke(_: &(), input: &mut DataChunkHandle, output: &mut dyn WritableVector) -> Result<(), Box<dyn Error>> {
        ...
        for i in 0..len {
            ...
            match (parse_csv_f64(&raw_v), parse_csv_dates(&raw_d)) {
                (Ok(values), Ok(dates)) if values.len() == dates.len() && !values.is_empty() => {
                    let d0 = dates[0];
