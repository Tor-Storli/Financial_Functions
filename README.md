...
[dependencies]
duckdb = { version = "1.10502.0", features = ["vtab-loadable", "vscalar"] }
...
::: {.callout-note}
The `duckdb-rs` crate encodes the DuckDB version in its semver string. Version `1.10502.0` corresponds to **DuckDB v1.5.2**. Always keep `duckdb`, `duckdb-loadable-macros`, and `libduckdb-sys` on the same version.  
...
5. **Version pinning** — `duckdb`, `duckdb-loadable-macros`, and `libduckdb-sys` must all be on the **same version string**. The version encodes the DuckDB release: `1.10502.0` = DuckDB v1.5.2.
